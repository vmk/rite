...
package io.github.vmk.rite.relic.persistence.mongo;
...
import io.github.vmk.rite.relic.exceptions.RelicRuntimeException;
import io.github.vmk.rite.relic.persistence.RelicStore;
...
public class MongoStore implements RelicStore {
	...
	private boolean collectionExists(final String collectionName, Set<String> collectionNames) {
	...
	public MongoStore(String host, int port, String dbname, String collection) throws RelicException {
		try {
			...
			if (collectionExists(collection, db.getCollectionNames())) {
	...
	public MongoStore(String host, int port, String dbname, String collection, String user, String pass)
			throws RelicException {
		try {
			mongo = new Mongo(host, port);
			db = mongo.getDB(dbname);
	...
	public Relic getRelic(String relicId) {
		...
		if (find.size() == 1) {
			...
			throw new RelicRuntimeException(
					"The relic collection is possibly corrupt, more than one entry was found for the identifier ["
							+ relicId + "].");
		} else {
			return null;
		}
	...
	public ResolverDescriptor getResolverDescriptor(String relicId, String environment) {
		...
		if (find.size() == 1) {
			...
			throw new RelicRuntimeException(
					"The relic collection is possibly corrupt, more than one entry was found for the identifier ["
							+ relicId + "] and environment [" + environment + "].");
		} else {
			return null;
		}
	}
